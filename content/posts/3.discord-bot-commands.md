...
::code-block{title="src/commands/Command.ts"}
```typescript
...
::code-block{title="src/commands/Ping.command.ts"}
...
::code-block{title="src/commands/Say.command.ts"}
```typescript
...
::code-block{title="src/commands/CommandFactory.ts"}
```typescript
import { Client, Message } from 'discord.js';
